...
func HandleLogin(db *sql.DB) http.HandlerFunc {
	return http.HandlerFunc(func(wr http.ResponseWriter, req *http.Request) {
		...
		type loginRequest struct {
			Username string `json:"username,omitempty"` // better
...
func CheckPass(db *sql.DB) http.HandlerFunc {
	return http.HandlerFunc(func(wr http.ResponseWriter, req *http.Request) {
		userDetails, _ := UserFromSession(req)
		q := store.New(db)
		user, err := q.GetUser(req.Context(), userDetails.UserID)
		if errors.Is(err, sql.ErrNoRows) {
			api.JSONError(wr, http.StatusForbidden, "Please Check Your Password again")
			return
		}
		api.JSONMessage(wr, http.StatusOK, fmt.Sprintf("Login Succesful, Redirecting to %s", user.Name)) // add redirect to the dashboard instead of user.Name
	})
}
